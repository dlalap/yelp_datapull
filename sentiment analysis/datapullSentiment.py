...
summaryList = resultsWorkbook.create_sheet(title="Sentiments")
...
# print(ratingResults)
# print(dateResults)
for reviewText in reviewResults:
	analyzeThis(reviewText)
	compileSentiments()

for word in wordSentiment.keys():
	avgScores.update({word:avgWordValue(wordSentiment[word]['Score'])})
	avgMagnitudes.update({word:sumWordValue(wordSentiment[word]['Magnitude'])})

# Creating top row labels

summaryList['A1'] = 'Word'
summaryList['B1'] = 'Word Score (out of 5)'
summaryList['C1'] = 'Word Emotionality (out of 5)'
summaryList['D1'] = 'Number of Occurances'

for eachWord in range(len(wordSentiment.keys())):
	wordIndex = 'A' + str(eachWord + 2)
	scoreIndex = 'B' + str(eachWord + 2)
	magnitudeIndex = 'C' + str(eachWord + 2)
	occurancesIndex = 'D' + str(eachWord + 2)
	summaryList[wordIndex] = wordSentiment.keys()[eachWord]
	summaryList[scoreIndex] = 1 + 2*(1 + avgScores[wordSentiment.keys()[eachWord]])
	summaryList[magnitudeIndex] = avgMagnitudes[wordSentiment.keys()[eachWord]]
	summaryList[occurancesIndex] = occurances[wordSentiment.keys()[eachWord]]
for item in range(len(dateResults)):
	...
	# printEntity(analyzeThis(reviewResults[item]))
...
print(avgScores)
print(avgMagnitudes)
resultsWorkbook.save(outputName)
